...
export const ForecastVsActualsTab: React.FC<ForecastVsActualsTabProps> = ({
  ...
  const chartData = useMemo(() => {
    ...
    const { specificFcMonthMap, fcSeedBase, fcLastIn, fcLastOut, _matchFcs } = (() => {
      ...
      // Collect all matching forecastStore entries
      const matchFcs: BaseForecast[] = [];
      for (const [key, bf] of forecastStore.entries()) {
        const p = key.split('|');
        if (p[0] !== seg)              continue;
  ...
  const multiChartData = useMemo((): MultiChartRow[] => {
    ...
    const fcRows: MultiChartRow[] = comparisonRows.map(row => {
      ...
      if (specificFcMonthMap) {
        ...
        // Fall back to baseForecast for aggregate scale
  ...
  const summaryMape = useMemo(() => {
    ...
    for (const bf of forecastStore.values()) {
      const { segment, product, productL2, channel, channelL2 } = bf.cohort;
      ...
      if (segMatch && prodMatch && chanMatch) {
        matching.push(bf);
      }
    }
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-3 text-center">
                          <span
                            className={`inline-block px-2 py-0.5 rounded-full text-xs font-bold tabular-nums cursor-help ${scoreBg(c.overallScore)}`}
