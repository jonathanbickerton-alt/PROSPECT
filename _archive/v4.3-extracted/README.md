...
| @google/genai | 1 | Google Gemini API client (AutoML challenger analysis) |
...
The app has one optional environment variable:

| Variable | Required | Description |
|---|---|---|
| `GEMINI_API_KEY` | Optional | Google Gemini API key powering the AutoML Challenger Analysis feature. If omitted the feature is disabled; all other functionality works normally. |

> **Important:** `GEMINI_API_KEY` is baked into the static bundle at **build time** by Vite (see `vite.config.ts`). It must therefore be supplied during the Docker build step, not at runtime.

To include the key in the build:

```bash
docker build \
  --build-arg GEMINI_API_KEY=your_key_here \
  -t REGION-docker.pkg.dev/YOUR_PROJECT_ID/prospect/app:v4.3 .
```

And add the following two lines to `Dockerfile` Stage 1, immediately before the `RUN npm run build` line:
