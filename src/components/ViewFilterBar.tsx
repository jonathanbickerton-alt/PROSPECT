...
export function ViewFilterBar({
  ...
  return (
    <div className="bg-slate-800 text-white px-6 py-2 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs shrink-0">
      ...
      {/* Product — only shown when mapped */}
      {products.length > 0 && (
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] uppercase tracking-wide text-slate-500">Product</span>
          <div className="relative">
            <select
      ...
      {/* Channel — only shown when mapped */}
      {channels.length > 0 && (
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] uppercase tracking-wide text-slate-500">Channel</span>
          <div className="relative">
            <select
              value={filter.channel}
              onChange={e => onChange({ ...filter, channel: e.target.value })}
              className={selectCls}
            >
              <option value="All">All</option>
              {channels.map(c => (
